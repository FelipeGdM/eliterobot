...
class _EC(
        ...
        while True:
            ...
            if result != self.RobotState.PLAY:
                if result != self.RobotState.STOP:
                    str_ = [
                        "",
                        "state of robot in the pause",
                        "state of robot in the emergency stop",
                        "",
                        "state of robot in the error",
                        "state of robot in the collision",
                    ]
    ...
    def robot_servo_on(self) -> bool:
        ...
        if self.TT_state:
            self.logger.debug(
                "The TT state is enabled, and the TT cache is automatically cleared"
            )
            time.sleep(0.5)
        ...
        if str(robot_mode) == "2":
            ...
            if state == 1:
                ...
                while 1:
                    ...
                    if clear_num > 4:
                        self.logger.error(
                            "The Alarm can't clear,Please check the robot state"
                        )
                        return False
